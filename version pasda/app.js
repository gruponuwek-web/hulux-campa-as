...
let state = {
  campanas: [],
  reportes: {}, // { campanaId: [ { semana, gasto, contactos, soporte, leads, cierres:{mun:n}, alcance, impresiones } ] }
};
...
let dashMesSeleccionado = null; // 'YYYY-MM'
...
async function cargarState() {
  ...
  if (res && res.ok) {
    ...
    guardarStateLocal(); // backup
    mostrarSyncStatus('ok');
  } else {
    gsOnline = false;
    cargarStateLocal();  // fallback offline
...
function showScreen(id) {
  ...
  const map = { campanas: 0, reporte: 1, dashboard: 2 };
...
(async () => {
  ...
  // Cargar datos (Sheets o localStorage)
  await cargarState();
  renderCampanas();
})();
